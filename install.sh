...
if [ ! -d /opt/lexicon ] && [ ! -f /etc/systemd/system/lexicon.service ]; then
	...
	if [[ $uninstall == [Yy] ]]; then
		echo "Uninstalling Lexicon..."
		systemctl stop lexicon.service
		systemctl disable lexicon.service
		rm -f /etc/systemd/system/lexicon.service
		rm -rf /opt/lexicon/
		rm -f /usr/local/bin/lexicon
		echo "Lexicon uninstalled successfully."
	else
